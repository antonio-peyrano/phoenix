    ...
    class evaluaciones
        {
             ...
             * y funcionalidades de la interfaz del modulo de evaluaciones.
             */
            
            //ATRIBUTOS APLICABLES AL MODULO catEvaluaciones.php
            private $Condicionales = "";
            private $Sufijo = "eva_";
            private $idUsuario = NULL;
            private $idCedula = NULL;
            private $Fecha = "";
            private $Consulta = 'SELECT idEvaluacion, Folio, Fecha, if(Status=1,"Activa",if(Status=3,"Terminada",if(Status=1,"Eliminada","Indefinido"))) AS Status FROM opEvaluaciones WHERE 1=1';
            //FIN DE DECLARACION DE ATRIBUTOS APLICABLES AL MODULO catEvaluaciones.php
            
            //ATRIBUTOS APLICABLES AL MODULO opEvaluaciones.php
            private $idEvaluacion = NULL;
            private $cntView = 0;
            //FIN DE DECLARACION DE ATRIBUTOS APLICABLES AL MODULO opEvaluaciones.php
            
            public function __construct()
                {
                    //Declaracion de constructor (VACIO)
                    $this->setFechaRegistro(); //Se genera la fecha de registro.
                    $this->setIDUsuario(); //Se obtiene el ID del usuario activo.                    
                    }

            //PROCEDIMIENTOS APLICABLES AL MODULO catEvaluaciones.php                    
            public function setIDUsuario()
                {
                    /*
                     * Esta funcion establece el valor del ID de Usuario con sesion activa.
                     */
                    if(!isset($_SESSION))
                        {
            ...
            public function getIDUsuario()
                {
                    /*
                     * Esta funcion retorna el valor del ID del usuario con sesion activa.
                     */
            ...
            public function evaluaCondicion()
                {
                    ...
                    if($this->getidUsuario()!="-1")
                        {                            
                            $this->Condicionales .= ' AND idUsuario LIKE \'%'.$this->getIDUsuario().'%\'';
                            }
                                                
                    return $this->Condicionales;                            
                    }                    
            //FIN DE DECLARACION DE PROCEDIMIENTOS APLICABLES AL MODULO catEvaluaciones.php
            
            //PROCEDIMIENTOS APLICABLES AL MODULO opEvaluaciones.php.
            ...
            public function getRegistro($idRegistro)
                {
                    ...
                    $Consulta= 'SELECT idEvaluacion, idCedula, idUsuario, Folio, Fecha, Status FROM opEvaluaciones WHERE idEvaluacion='.$idRegistro; //Se establece el modelo de consulta de datos.
            ...
            //FIN DE DECLARACION DE PROCEDIMIENTOS APLICABLES AL MODULO opEvaluaciones.php            
            }
?>