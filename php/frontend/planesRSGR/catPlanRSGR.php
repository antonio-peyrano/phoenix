 ...
 */
    ...
    $sufijo= "psr_"; //Variable de control de sufijo de identificadores.
    $Inicio = 0;
    $Pagina = 0;
    $DisplayRow = 10;
    ...
    if(isset($_GET['nomclave']))
        {
             ...
             */
            $condicionales= 'Clave LIKE \'%'.$_GET['nomclave'].'%\'';
            }

    if(isset($_GET['nomriesgo']))
        {
            /*
             * Si el archivo ha sido llamado como una referencia con parametros.
             */
            $condicionales= 'Riesgo LIKE \'%'.$_GET['nomriesgo'].'%\'';
            }
    ...
    if($condicionales=="")
        {
            ...
            $consulta= "SELECT idPlanRSGR, Clave, Riesgo, Status FROM opPlanesRSGR WHERE Status=0 ORDER BY idPlanRSGR"." limit ".$Inicio.",".$DisplayRow; //Se establece el modelo de consulta de datos.
            }  
    else 
        {
            //En caso de contar con el criterio de filtrado.
            $consulta= "SELECT idPlanRSGR, Clave, Riesgo, Status FROM opPlanesRSGR WHERE Status=0 AND " .$condicionales. " ORDER BY idPlanRSGR"." limit ".$Inicio.",".$DisplayRow; //Se establece el modelo de consulta de datos.
            }  
    ...
    function constructor($dataset,$sufijo)
        {        
            ...
            $objGrid = new myGrid($dataset, 'Catalogo de Planes RSGR', $sufijo, 'idPlanRSGR');
