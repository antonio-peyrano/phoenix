 ...
 */
    ...
    $sufijo= "gas_"; //Variable de control de sufijo de identificadores.
    $Inicio = 0;
    $Pagina = 0;
    $DisplayRow = 10;
    ...
    if(isset($_GET['gasperiodo']))
        {
            /*
             * Si el archivo ha sido llamado como una referencia con parametros.
             */
            $condicionales= 'Periodo LIKE \'%'.$_GET['gasperiodo'].'%\'';
            }
            ...
            if($_GET['nomidentidad']!="-1")
                {
                    ...
                    $condicionales= $condicionales.'opProgGas.idEntidad LIKE \'%'.$_GET['nomidentidad'].'%\'';
                }
            }
    ...
    if($condicionales=="")
        {
            ...
            $consulta= "SELECT idProgGas, opProgGas.idEntidad, Entidad, opProgGas.Status FROM (opProgGas INNER JOIN catEntidades ON catEntidades.idEntidad = opProgGas.idEntidad) WHERE opProgGas.Status=0 ORDER BY idEntidad"." limit ".$Inicio.",".$DisplayRow; //Se establece el modelo de consulta de datos.
            }  
    else 
        {
            //En caso de contar con el criterio de filtrado.
            $consulta= "SELECT idProgGas, opProgGas.idEntidad, Entidad, opProgGas.Status FROM (opProgGas INNER JOIN catEntidades ON catEntidades.idEntidad = opProgGas.idEntidad) WHERE opProgGas.Status=0 AND " .$condicionales. " ORDER BY idEntidad"." limit ".$Inicio.",".$DisplayRow; //Se establece el modelo de consulta de datos.
            }  
    ...
    function constructor($dataset,$sufijo)
        {        
            ...
            $objGrid = new myGrid($dataset, 'Presupuesto de Gasolina', $sufijo, 'idProgGas');
