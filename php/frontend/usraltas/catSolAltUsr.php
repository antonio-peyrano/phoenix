 ...
 */
    ...
    $sufijo= "sau_"; //Variable de control de sufijo de identificadores.
    $Inicio = 0;
    $Pagina = 0;
    $DisplayRow = 10;
    ...
    if($condicionales=="")
        {
            ...
            $consulta= 'SELECT idUsrtmp, Nivel, Usuario, Clave, Correo, opUsrTemp.Status FROM opUsrTemp INNER JOIN catNiveles ON catNiveles.idNivel = opUsrTemp.idNivel WHERE opUsrTemp.Status=0'." limit ".$Inicio.",".$DisplayRow; //Se establece el modelo de consulta de datos.
            }  
    else 
        {
            //En caso de contar con el criterio de filtrado.
            $consulta= 'SELECT idUsrtmp, Nivel, Usuario, Clave, Correo, opUsrTemp.Status FROM opUsrTemp INNER JOIN catNiveles ON catNiveles.idNivel = opUsrTemp.idNivel WHERE opUsrTemp.Status=0 AND ' .$condicionales." limit ".$Inicio.",".$DisplayRow; //Se establece el modelo de consulta de datos.
            }  
    ...
    function constructor($dataset,$sufijo)
        {        
            ...
            $objGrid = new myGrid($dataset, 'Usuarios para dar de alta', $sufijo, 'idUsrtmp');
