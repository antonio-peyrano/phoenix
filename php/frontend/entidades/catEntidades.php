 ...
 */
    ...
    $sufijo= "ent_"; //Variable de control de sufijo de identificadores.
    $Inicio = 0;
    $Pagina = 0;
    $DisplayRow = 10;
    ...
    if(isset($_GET['nomentidad']))
        {
            /*
             * Si el archivo ha sido llamado como una referencia con parametros.
             */
            $condicionales= 'Entidad LIKE \'%'.$_GET['nomentidad'].'%\'';
            }

    if(isset($_GET['nomidtentidad']))
        {
            /*
             * Si el archivo ha sido llamado como una referencia con parametros.
             */
            if($_GET['nomidtentidad']!="-1")
                {
                    if($condicionales!='')
                        {
                    ...
                    $condicionales= $condicionales.'catEntidades.idTEntidad LIKE \'%'.$_GET['nomidtentidad'].'%\'';
                    }
            }
    ...
    if($condicionales=="")
        {
            ...
            $consulta= "SELECT idEntidad, Entidad, TEntidad, catEntidades.Status FROM catEntidades INNER JOIN catTEntidades ON catEntidades.idTEntidad = catTEntidades.idTEntidad WHERE catEntidades.Status=0"." limit ".$Inicio.",".$DisplayRow; //Se establece el modelo de consulta de datos.
            }  
    else 
        {
            //En caso de contar con el criterio de filtrado.
            $consulta= "SELECT idEntidad, Entidad, TEntidad, catEntidades.Status FROM catEntidades INNER JOIN catTEntidades ON catEntidades.idTEntidad = catTEntidades.idTEntidad WHERE catEntidades.Status=0 AND " .$condicionales." limit ".$Inicio.",".$DisplayRow; //Se establece el modelo de consulta de datos.
            }  
    ...
    function constructor($dataset,$sufijo)
        {        
            ...
            $objGrid = new myGrid($dataset, 'Catalogo de Entidades', $sufijo, 'idEntidad');
