...
function habFactor()
	{
		...
		document.getElementById('ffa_Guardar').style.display="block";
		document.getElementById('ffa_Borrar').style.display="none";
		document.getElementById('ffa_Editar').style.display="none";
		}
		            			...
		            			if(result)
		            				{
		            					...
		            					cargar('./php/backend/foda/factores/borrar.php','?id='+e.target.id.substring(11),'sandbox');
		            					}			            					
		            			}
	            		});
    			}
    	...
    	if(e.target.id.substring(0,7) == "ffa_add")
    		{
    			...
    			cargar('./php/frontend/foda/factores/opFactores.php','?id=-1&ffacedula='+document.getElementById('ffacedula').value.toString()+'&view=0','sandbox');
    			}
    });                 
});
    	...
    	if(e.target.id.substring(0,14) == "ffa_visualizar")
    		{
    			...
    			cargar('./php/frontend/foda/factores/opFactores.php','?id='+e.target.id.substring(15)+'&view=1','sandbox');
    			}
    });                 
});
    	...
    	if(e.target.id.substring(0,8) == "ffa_edit")
    		{
    			...
    			cargar('./php/frontend/foda/factores/opFactores.php','?id='+e.target.id.substring(9)+'&view=0','sandbox');
    			}
    });                 
});
		...
		$(document).ready(function()
			{
	    		$("div").click(function(e)
	    			{
	    		     	e.stopPropagation();
	    		        if(e.target.id == "ffa_Volver")
	    		        	{
	    		            	//En caso de coincidir el id con la accion volver.
	    		        		cargar('./php/frontend/foda/factores/busFactores.php','','sandbox');
	    		            	}
	    				});                 
				});
	    				            	...
	    				            	callback: function (result)
	    				            		{
	    				            			if(result)
	    				            				{
	    				            					//EL USUARIO DECIDE BORRAR EL REGISTRO.
	    				            					cargar('./php/backend/foda/factores/borrar.php','?id='+document.getElementById('idFactor').value.toString(),'sandbox');
	    				            					}			            					
	    				            			}
	    			            		});
	    			    		}
