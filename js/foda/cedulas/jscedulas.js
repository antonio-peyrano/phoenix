...
function guardarCedula(url,parametro){
	...
	if(document.getElementById("idEntfoda").value.toString() == "-1")
		{
			//En caso de no ocurrir un error de validación, se asigna el valor de paso.
			error= error+1;
			}
...
function habCedula()
	{
		...
		document.getElementById('fce_Guardar').style.display="block";
		document.getElementById('fce_Borrar').style.display="none";
		document.getElementById('fce_Editar').style.display="none";		
		}
...
$(document).ready(function() {
    $("div").click(function(e){
    	e.stopPropagation();
    	if(e.target.id.substring(0,10) == "fce_delete")
    		{
		            			...
		            			if(result)
		            				{
		            					...
		            					cargar('./php/backend/foda/cedulas/borrar.php','?id='+e.target.id.substring(11),'sandbox');
		            					}			            					
		            			}
	            		}); 		
    			}
...
$(document).ready(function() {
    $("div").click(function(e){
    	e.stopPropagation();
    	if(e.target.id.substring(0,7) == "fce_add")
    		{
    			//En caso de coincidir el id con la accion agregar.
    			cargar('./php/frontend/foda/cedulas/opCedulas.php','?id=-1&view=0','sandbox');
    			}
    });                 
});
...
$(document).ready(function() {
    $("div").click(function(e){
    	e.stopPropagation();
    	if(e.target.id.substring(0,14) == "fce_visualizar")
    		{
    			//En caso de coincidir el id con la accion visualizar.
    			cargar('./php/frontend/foda/cedulas/opCedulas.php','?id='+e.target.id.substring(15)+'&view=1','sandbox');
    			}
    });                 
});
...
$(document).ready(function() {
    $("div").click(function(e){
    	e.stopPropagation();
    	if(e.target.id.substring(0,8) == "fce_edit")
    		{
    			//En caso de coincidir el id con la accion editar.
    			cargar('./php/frontend/foda/cedulas/opCedulas.php','?id='+e.target.id.substring(9)+'&view=0','sandbox');
    			}
    });                 
});
		...
		$(document).ready(function()
			{
	    		$("div").click(function(e)
	    			{
	    		     	e.stopPropagation();
	    		        if(e.target.id == "fce_Volver")
	    		        	{
	    		            	//En caso de coincidir el id con la accion volver.
	    		        		cargar('./php/frontend/foda/cedulas/busCedulas.php','','sandbox');
	    		            	}
	    				});                 
				});
	    				            	...
	    				            	callback: function (result)
	    				            		{
	    				            			if(result)
	    				            				{
	    				            					//EL USUARIO DECIDE BORRAR EL REGISTRO.
	    				            					cargar('./php/backend/foda/cedulas/borrar.php','?id='+document.getElementById('idCedula').value.toString(),'sandbox');
	    				            					}			            					
	    				            			}
	    			            		});
	    			    		}
	    				            			...
	    				            			if(result)
	    				            				{
	    				            					...
	    				            					guardarCedula('./php/backend/foda/cedulas/guardar.php','?id='+document.getElementById('idCedula').value.toString()+'&folio='+document.getElementById('Folio').value.toString()+'&identfoda='+document.getElementById('idEntfoda').value.toString()+'&fecha='+document.getElementById('Fecha').value.toString()+'&horizonte='+document.getElementById('Horizonte').value.toString()+'&status='+document.getElementById('Status').value.toString());
	    				            					}			            					
	    				            			}
	    				        		});			        		
	    						}
