    ...
    if($condicionales=="")
        {
            ...
            $consulta= "SELECT idPrograma, Nomenclatura, Programa, Entidad, opProgramas.Status FROM (opProgramas INNER JOIN catEntidades ON catEntidades.idEntidad = opProgramas.idEntidad) WHERE opProgramas.Status=0 AND opProgramas.Periodo=".$Periodo." ORDER BY idPrograma"; //Se establece el modelo de consulta de datos.
            }  
    else 
        {
            //En caso de contar con el criterio de filtrado.
            $consulta= "SELECT idPrograma, Nomenclatura, Programa, Entidad, opProgramas.Status FROM (opProgramas INNER JOIN catEntidades ON catEntidades.idEntidad = opProgramas.idEntidad) WHERE opProgramas.Status=0 AND " .$condicionales. " AND opProgramas.Periodo=".$Periodo." ORDER BY idPrograma"; //Se establece el modelo de consulta de datos.
            }  
    ...
    function dgDatos($dataset)
        {
             ...
             */
            global $sufijo, $Periodo; //Acceso a la variable de sufijo.
                    ...
                    while ($record) 
                        {
                            ...
                            $rows = $rows.'<td id="reg_'.$record['idPrograma'].'" width= "90"><img id="'.$sufijo.'visualizar_'.$record['idPrograma'].'"align= "middle" src= "./img/grids/view.png" width= "25" height= "25" alt= "Visualizar"/></td></tr>';  
    ...
    function loadInfo($idRegistro, $titulo)        
        {
             ...
             */
              ...
              */
             $objConexion= new mySQL_conexion($username, $password, $servername, $dbname); //Se crea el objeto de la clase a instanciar.
             $consulta= "SELECT idEstOpe AS ID, Nomenclatura, EstOpe AS Objetivo, Periodo, Status FROM catEstOpe WHERE Status=0 AND idEstOpe=".$idRegistro; //Se establece el modelo de consulta de datos.
             ...
             if($Registro)
                {
                    ...
                    echo'               <tr class= "dgHeader"><td alignt= "left" colspan= 2"><a href="#" onclick="cargar(\'./php/frontend/consulplan/conEstOpe.php\',\'?idobjope='.$idObjOpe.'&idobjest='.$idObjEst.'\',\'escritorio\');"><img align= "right" src= "./img/grids/volver.png" width= "25" height= "25" alt= "Volver" id= "btnVolver"/></a></td></tr>
                            </table></div><br>';
                    }
            /*
             ...
             */                    
             $objConexion= new mySQL_conexion($username, $password, $servername, $dbname); //Se crea el objeto de la clase a instanciar.
             $consulta= "SELECT  Enero, Febrero, Marzo, Abril, Mayo, Junio, Julio, Agosto, Septiembre, Octubre, Noviembre, Diciembre FROM opProgEst WHERE Status=0 AND idEstOpe=".$idRegistro; //Se establece el modelo de consulta de datos.
             $dsPadre = $objConexion -> conectar($consulta); //Se ejecuta la consulta.
             $dsTitulos = $dsPadre;
             $Registro = @mysql_fetch_array($dsPadre,MYSQLI_ASSOC); //Se obtiene el registro que coincide con el criterio proporcionado.
             ...
             */
            $objConexion= new mySQL_conexion($username, $password, $servername, $dbname); //Se crea el objeto de la clase a instanciar.
            $consulta= "SELECT Enero, Febrero, Marzo, Abril, Mayo, Junio, Julio, Agosto, Septiembre, Octubre, Noviembre, Diciembre FROM opEjecEst WHERE Status=0 AND idEstOpe=".$idRegistro; //Se establece el modelo de consulta de datos.
            $dsPadre = $objConexion -> conectar($consulta); //Se ejecuta la consulta.
            $dsCampos = $dsPadre;
            $Registro = @mysql_fetch_array($dsPadre,MYSQLI_ASSOC); //Se obtiene el registro que coincide con el criterio proporcionado.
             ...
             */
            $objConexion= new mySQL_conexion($username, $password, $servername, $dbname); //Se crea el objeto de la clase a instanciar.
            $consulta= "SELECT Enero, Febrero, Marzo, Abril, Mayo, Junio, Julio, Agosto, Septiembre, Octubre, Noviembre, Diciembre FROM opEficEst WHERE Status=0 AND idEstOpe=".$idRegistro; //Se establece el modelo de consulta de datos.
            $dsPadre = $objConexion -> conectar($consulta); //Se ejecuta la consulta.
            $dsCampos = $dsPadre;
            $Registro = @mysql_fetch_array($dsPadre,MYSQLI_ASSOC); //Se obtiene el registro que coincide con el criterio proporcionado.
    ...
    function constructor($dataset)
        {        
            ...
            echo'       <center>';
                        ...
                        loadInfo($idEstOpe,'Información de la Estrategia Operativa');
                        dgCabeceras($dataset,'Programas');//Funcion para crear la cabecera de columnas de datos.
