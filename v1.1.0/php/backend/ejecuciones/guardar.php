 ...
 */
    ...
    if(isset($_GET['id'])&&isset($_GET['idactividad'])&&isset($_GET['idprograma'])&&isset($_GET['cantidad'])&&isset($_GET['idmes'])&&isset($_GET['monto'])&&isset($_GET['periodo'])&&isset($_GET['status']))
        {
            /*
    ...
    function updt_opEjecActAnual()
        {
            ...
            $RegPrgActUnit = @mysql_fetch_array($dsPrgActUnit,MYSQLI_ASSOC);
            ...
            $RegEjcActUnit = @mysql_fetch_array($dsEjcActUnit,MYSQLI_ASSOC);
            ...
            $RegActividades = @mysql_fetch_array($dsActividades,MYSQLI_ASSOC);
            ...
            $RegProgramas = @mysql_fetch_array($dsProgramas,MYSQLI_ASSOC);
            ...
            $dsProgramas = $objConexion -> conectar($consulta); //Se ejecuta la consulta.
            $RegEstOpe = @mysql_fetch_array($dsProgramas,MYSQLI_ASSOC);
            ...
            while($RegEstOpe)
                {
                    ...
                    $RegProgEst = @mysql_fetch_array($dsProgEst,MYSQLI_ASSOC);
                    ...
                    $RegEjecEst = @mysql_fetch_array($dsEjecEst,MYSQLI_ASSOC);
            ...
            $RegObjOpe = @mysql_fetch_array($dsObjOpe,MYSQLI_ASSOC);
            ...
            while($RegObjOpe)
                {
                    ...
                    $RegProgOO = @mysql_fetch_array($dsProgOO,MYSQLI_ASSOC);
                    ...
                    $RegEjecOO = @mysql_fetch_array($dsEjecOO,MYSQLI_ASSOC);
            ...
            if($idEjecucion != null)
                {
                     ...
                     */
                    $consulta = 'UPDATE opEjecuciones SET Cantidad=\''.$Cantidad.'\', Mes=\''.$Mes.'\', Monto=\''.$Monto.'\', idActividad=\''.$idActividad.'\', Periodo=\''.$Periodo.'\', Status=\''.$Status.'\' WHERE idEjecucion='.$idEjecucion; //Se establece el modelo de consulta de datos.
                    $dataset = $objConexion -> conectar($consulta); //Se ejecuta la consulta.
            
                    updt_opEjecAct($Mes); //Se llama la función de actualización de ejecucion a nivel de actividad.
                    updt_opEjecActAnual(); //Se llama a la función de calculo global.
                    }
            else
                {
                    /*
                     ...
                     */
                    $consulta = 'INSERT INTO opEjecuciones (Cantidad, Mes, Monto, idActividad, Periodo) VALUES ('.'\''.$Cantidad.'\',\''.$Mes.'\',\''.$Monto.'\', \''.$idActividad.'\', \''.$Periodo.'\')'; //Se establece el modelo de consulta de datos.
                    $dataset = $objConexion -> conectar($consulta); //Se ejecuta la consulta.
